...
export default function Home({ user, onOpenAuth, onLogout }) {
  ...
  const handleOrder = (item, size, qty, totalPrice) => {
    ...
    setCart([...cart, { cartId: Date.now(), ...item, size, qty, totalPrice }]);
  ...
  // إرسال الطلب نهائياً للباك إند وحفظه بقاعدة البيانات على Render
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-slate-100 font-sans text-slate-800">
      {/* شريط التنقل العلوي مع تمرير حالة السلة وزر الخروج */}
      <nav className="bg-white shadow-md p-4 flex justify-between items-center px-6">
        <h1 className="text-xl font-black text-emerald-700">بهاراتي | Baharati</h1>
        ...
        <div className="flex items-center gap-4">
          ...
          {user ? (
            <div className="flex items-center gap-3">
              <span className="text-sm font-bold text-slate-700">مرحباً، {user.username}</span>
              <button 
                onClick={onLogout}
      ...
      {/* نافذة سلة المشتريات الجانبية المحمية */}
      {isCartOpen && (
            ...
            <div>
              <div className="flex justify-between items-center border-b pb-4 mb-4">
                <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                  <ShoppingCart className="w-5 h-5 text-emerald-600" /> سلة المشتريات
                </h2>
              ...
              <div className="max-h-[60vh] overflow-y-auto space-y-3">
                {cart.length === 0 ? (
                  <p className="text-sm text-slate-500 text-center py-10">السلة فارغة حالياً.</p>
                ) : (
                  cart.map((item, index) => (
                    <div key={item.cartId || index} className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border">
                      <div>
                        <h4 className="font-bold text-sm text-slate-800">{item?.name || 'منتج'}</h4>
                        <p className="text-xs text-slate-500">
                          الكمية: {item?.qty || 1} | الحجم: {item?.size || '-'} كيلو
                        </p>
                        <p className="text-xs font-bold text-emerald-600 mt-1">
                          {item?.totalPrice ? item.totalPrice.toFixed(2) : '0.00'} د.أ
                        </p>
                      </div>
                      <button 
              ...
              <div className="border-t pt-4 space-y-4">
                <div className="flex justify-between font-bold text-base">
                  ...
                  <span className="text-emerald-700">
                    {cart.reduce((sum, item) => sum + (item?.totalPrice || 0), 0).toFixed(2)} د.أ
                  </span>
                </div>
                <button 
                ...
                >
                  <CheckCircle2 className="w-5 h-5" /> {loadingCheckout ? 'جارِ إرسال الطلب...' : 'إتمام الطلب وإرساله'}
                </button>
              </div>
            )}
          </div>
